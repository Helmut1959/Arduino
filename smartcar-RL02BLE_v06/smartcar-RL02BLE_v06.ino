...
int epsilon [5]= {100,100,100,100,100};  // Anfangsbelegung je Zustand
...
//    forward();  //10.2.19: Forward soll stärkeren Effekt haben 
    delay(100);
    break;
  case 5:    // Rückwärts
    back();
  ...
  int random_number = random(0,100);  // Random number between 0 and 100
    ...
    if (alpha_opt < 30 ) {  //Abfangen, wenn zu weit links
      ...
      distance_t0 = distance;
       ...
       if (random_number < epsilon[s_t]) {  // Zufaellige Aktion
        a_t = random(0,6);    //Zufallszahlen 0..5
        epsilon[s_t]= epsilon[s_t]-5; //Epsilon nur bei Random verringern
...
void write_to_EEPROM() {
  int eeAddress = 0;
   ...
   for (int i=0; i < 5; i++) {
        EEPROM.put(eeAddress,epsilon[i]);  
...
void read_from_EEPROM() {
  int eeAddress = 0;
   ...
   for (int i=0; i < 5; i++) {
        EEPROM.get(eeAddress,epsilon[i]);  
        eeAddress += sizeof(int); 
   }
   for (int i = 0; i < 5; i++) {
     for (int j = 0; j < 6; j++) {    
